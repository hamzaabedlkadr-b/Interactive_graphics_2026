...
- Manual triangulated mesh/UV exhibit with vertex markers, wireframe overlay, and normal-vector rods to show the surfaces, triangle meshes, UV mapping, and normal lessons explicitly.
- Physics-based mass-spring cable simulation with gravity, damping, wind force, semi-implicit Euler integration, and floor collision.
...
Progress: Marco's robot hierarchy/detail pass is mostly complete. The robot arms now use staged JavaScript keyframe poses for approach, grip, lift, rotate, lower, release, and return. The models include base bolts, rotating rings, shoulder/elbow/wrist caps, hydraulic rods, local cables, nameplates, gripper pads, wrist lights, and stage lights. The main robot is centered in the glass inspection cell and synchronizes with a visible belt workpiece: the part waits on the belt, disappears when the claw closes, appears in the gripper, then reappears on a separate side drop table across the cell. The two robots use offset/mirrored cycles with different timing and placement.
...
See `COURSE_CONCEPT_MAP.md` for the detailed mapping between the professor's slide topics and the project implementation, including transformations, GPU/WebGL pipeline, textures, shading, triangle meshes, shadows, animation, physics-based simulation, and Three.js final considerations.
...
Walk mode can run in third-person avatar control or first-person walking. Use `Walk 3rd` / `Walk 1st` to switch the selected view before or during walking. The walking character is blocked by the main machines, walls, desks, racks, and conveyor, and slides along obstacles instead of passing through them. When you leave third-person view or press `Exit Walk`, the character stays parked where you left him and third-person view returns to that same character.
